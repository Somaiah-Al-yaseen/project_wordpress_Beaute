		...
		foreach ( $elements as &$el ) {
			...
			if ( current_action() === 'vamtam_after_content_import' ) {
				...
				if ( $el['elType'] === 'widget' && in_array( $el['widgetType'], [ 'loop-grid', 'loop-carousel' ] ) ) {
					foreach ( [ 'include', 'exclude' ] as $option ) {
						if ( ! empty( $el['settings']["post_query_{$option}_term_ids"] ) ) {
							foreach ( $el['settings']["post_query_{$option}_term_ids"] as $index => $cid ) {
								if ( isset( $map['terms'][ (int) $cid ] ) ) {
									$el['settings']["post_query_{$option}_term_ids"][ $index ] = strval( $map['terms'][ (int) $cid ] );
								}
							}
						}
					}
